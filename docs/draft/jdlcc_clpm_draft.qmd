...
\tikz{
    \node (x1) at (0,2) {$X_1$};
    \node[hidden] (x2) at (2,2) {$X_2$};
    \node (x3) at (4,2) {$X_3$};
    \node[hidden] (x4) at (6,2) {$X_4$};
    \node (x5) at (8,2) {$X_5$};
    \node (y1) at (0,0) {$Y_1$};
    \node[hidden] (y2) at (2,0) {$Y_2$};
    \node (y3) at (4,0) {$Y_3$};
    \node[hidden] (y4) at (6,0) {$Y_4$};
    \node (y5) at (8,0) {$Y_5$};
    \path (x1) edge (x2);
    \path (x2) edge (x3);
    \path (x3) edge (x4);
...
\tikz{
    ...
    \path (x3) edge [bend right=45, arrows = <->] (y3);
    \path (x5) edge [bend right=45, arrows = <->] (y5)
...
> We've written up ESC using legla cynicism as motivating example, but if we find it to be too complicated, we could also use the Airbnb paper for ESC (doesn't make sense for JDLCC though). It is nice because it has a DAG to motivate an estimation strategy with contemporaneous effects:
...
* RE
   * Addresses within-unit or within-period heterogeneity uncorrelated with regressors
   * Requires very strong assumptions in lagged DV setting
* FE
   * Addresses within-unit or within-period heterogeneity correlated with regressors
   * Introduces Nickell bias with lagged DVs
* Hybrid / Mundlak model (correlated RE)
