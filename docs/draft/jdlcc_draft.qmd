...
Researchers often assume there exists one "true causal effect" between two variables that corresponds to a particular potentially unknown lag structure. Simulations of data with true underlying causal effects existing at one time lag reveal that incorrect specifications of temporal lags in statistical models can produce estimates with severe bias including flipped signs. As Vaisey & Miles (XXXX) document, if  Equation 1 is the true reduced form model of interest ($x$ has only a contemporaneous effect) and you instead specify Equation 2 in your statistical model ($x$ with only a lagged effect), then $E(\beta^*) = -0.5\beta$, i.e., "a positive contemporaneous effect of $x$ gets transformed into an artifactually negative lagged effect" [Allison (2022)](https://statisticalhorizons.com/getting-the-lags-right-a-new-solution/).
