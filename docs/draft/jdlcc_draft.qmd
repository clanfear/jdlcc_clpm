...
Researchers often assume there exists one "true causal effect" between two variables that corresponds to a particular potentially unknown lag structure. Simulations of data with true underlying causal effects existing at one time lag reveal that incorrect specifications of temporal lags in statistical models can produce estimates with severe bias including flipped signs. As Vaisey & Miles (XXXX) document, if  Equation 1 is the true model (a contemporaneous effect of $x$) and you instead specify Equation 2 in your statistical model (a lagged effect of $x$), then $E(\beta^*) = -0.5\beta$, i.e., "a positive contemporaneous effect of $x$ gets transformed into an artifactually negative lagged effect" [Allison (2022)](https://statisticalhorizons.com/getting-the-lags-right-a-new-solution/).

\begin{align}\label{eq-vaiseymiles}
y &= \beta x_t + \alpha_i + e_{it} \\
y &= \beta^* x_{t-1} + \alpha_i + e_{it}
\end{align}
