...
Researchers often assume there exists one "true causal effect" between two variables that corresponds to a particular potentially unknown lag structure. Simulations of data with true underlying causal effects existing at one time lag reveal that incorrect specifications of temporal lags in statistical models can produce estimates with severe bias including flipped signs. As Vaisey & Miles (2017) document, if  Equation 1 is the true reduced form model of interest ($x$ has only a contemporaneous effect) and you instead specify Equation 2 in your statistical model ($x$ with only a lagged effect), then $E(\beta^*) = -0.5\beta$, i.e., "a positive contemporaneous effect of $x$ gets transformed into an artifactually negative lagged effect" [Allison (2022)](https://statisticalhorizons.com/getting-the-lags-right-a-new-solution/).
...
However, as we have stated earlier, there is generally not "one true causal effect" but an infinite number of potential estimands researchers may specify. In this regard, what is important is specifying the correct lag structure *for your chosen estimand*. Both equations make the strong assumption there is no reverse causality between $x_t$ and $y_t$. If the estimand is the effect of $x_{t-1} \rightarrow y_t$ and there is concerned about contemporaneous reverse causality, some auxiliary information is necessary such as an instrumental variable. If the estimand of interest is the lagged effect of $x_{t-1} \rightarrow y_t$ however, then the correct estimator for dealing with contemporaneous reverse causality is the cross-lagged model with a covariance between each $x_t$ and $y_t$ to address the nuisance reciprocality that may bias the estimate of interest [Allison (2022)](https://statisticalhorizons.com/getting-the-lags-right-a-new-solution/).
