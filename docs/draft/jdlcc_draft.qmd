...
Researchers often assume there exists one "true causal effect" between two variables that corresponds to a particular potentially unknown lag structure.
...
Simulations that generate data with true underlying causal effects existing at one particular time lag reveal that incorrect specifications of temporal lags in statistical models can produce estimates with severe bias including flipped signs. As Vaisey & Miles (2017) document, if  Equation 1 is the true reduced form model of interest ($x$ has only a contemporaneous effect) and you instead specify Equation 2 in your statistical model ($x$ with only a lagged effect), then $E(\beta^*) = -0.5\beta$, i.e., "a positive contemporaneous effect of $x$ gets transformed into an artifactually negative lagged effect" [Allison (2022)](https://statisticalhorizons.com/getting-the-lags-right-a-new-solution/).
...
This is possibly the least appreciated and most serious common issues facing CLPMs as it may produce misleading results that reject theoretically-motivated hypotheses---and provide evidence of the *opposite* proposition. Research that finds unexpectedly negative estimates from CLPMs when theory suggests positive effects may be the result of this bias.


A secondary issue if temporal order
...
However, as we have stated earlier, there is no "one true causal effect" but any number of potential estimands researchers may choose. In this regard, what is important is specifying the correct lag structure *for your chosen estimand*. Both equations make the strong assumption there is no reverse causality between $x_t$ and $y_t$. 
