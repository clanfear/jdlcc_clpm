...
Good empirical research starts with a strong theoretical foundation. It is from a clearly defined theoretical framework that observable implications can be deducted, and it is those observable implications which can then be contrasted against empirical data [@king1994designing]. Theory-testing empirical research is therefore conditional on a thorough and transparent theoretical model [@nagin2019real]. Given that most (all?) criminological theories presume causal processes, the first step should involve deriving a recursive model that accurately represents the theorized relationships---e.g., the theoretical model could be represented by a directed acyclic graph (DAG). DAGs can formally represent a nonparametric structural equation model, avoiding functional-form and distributional assumptions [@pearl2009causality], and are increasingly used to portray premised causal relationships in criminological theory [@lanfear2020broken, @nivette2025asessing, @oliveira2024aggressive]. The DAG should be all-encompassing and include all relevant theorized pathways, including unobserved mechanisms.

Once the theoretical model is specified, it should guide the selection of an appropriate estimator. When dealing with situations involving potential reciprocal relationships and/or reverse causality, we propose leveraging DAGs for theoretical models and drawing on the structural equation modelling (SEM) framework for estimation [@bollen2013eight]. A clear theoretical model is a necessary prerequisite for specifying an estimator. While  both diagrams will not be exactly the same, we suggest that the premised DAG inform the *structural* components of the SEM (i.e., one-directional arrows)---i.e., the structural part of the SEM should be non-recursive. For example, a key consideration in this process is the timing of effects: theorized effects can be contemporaneous (i.e., occur quickly) or lagged (i.e., unfold more gradually over time). However, while the DAG is a non-parametric representation of theorized causal relationships, the estimator needs to handle distributional and parametric assumptions and theoretical ambiguities. Even though this essentially makes the SEM a non-recursive model, covariances (i.e., bi-directional arrows) need to be included. Simultaneous equations should only be used only as a last resort, given their often tenuous theoretical grounding and the risk of overfitting.

## When is a CLPM needed?

A cross-lagged panel model is only needed when reciprocal effects over time are presumed in the theoretical model. When the researcher can theoretically assume no effects of the dependent variable on the independent variable over time, other statistical models will be more appropriate than a cross-lagged panel model. For example, research focused on the effects of a single-event exposure---a common topic in life-course criminology and the role of turning points, such as the long-term effects of school drop-out or marriage on offending [@sampson2016turning, @nguyen2018measurement]---would not benefit from cross-lagged panel models. 
