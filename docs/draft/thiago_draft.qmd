...
This section introduces two empirical examples that motivate our discussion on reciprocal relationships, reverse causality, and temporal ordering. The first is the relationship between occupational attainment and crime involvement, as highlighted by classic studies on developmental and life-course criminology [@sampson1992crime, @sampson1993crime]. The second is the relationship between perceptions of neighborhood disorder and fear of crime, often studied by neighborhood scholars [@brunton2011untangling, @jackson2004experience].
...
Good empirical research starts with a strong theoretical foundation. It is from a clearly defined theoretical framework that observable implications can be deducted, and it is those observable implications which can then be contrasted against empirical data [@king1994designing; @mayo2018statistical]. Theory-testing empirical research is therefore conditional on a thorough and transparent theoretical model [@nagin2019real]. Given that most criminological theories presume causal processes, the first step should normally involve deriving a non-recursive model that accurately represents the theorized relationships---e.g., the theoretical model could be represented by a directed acyclic graph (DAG). DAGs can formally represent a nonparametric structural equation model, avoiding functional-form and distributional assumptions [@pearl2009causality], and are increasingly used to portray premised causal relationships in criminological theory [@lanfear2020broken, @nivette2025asessing, @oliveira2024aggressive]. The DAG should be all-encompassing and include all relevant theorized pathways, including unobserved mechanisms.
...
The first estimator is @allison2017maximum's ML-SEM. XXX

```{tikz mlsem, fig.cap = "Maximum likelihood structural equation fixed-effects dynamic panel model (Allison et al., 2017)"}
...
\tikz{
    ...
    \path (alpha) edge [bend right=320, arrows = <->, gray, thin] (x3);
    \path (alpha) edge [bend right=320, arrows = <->, gray, thin] (x2);
    \path (alpha) edge [bend right=320, arrows = <->, gray, thin] (x1);
    \path (alpha) edge [bend right=300, arrows = <->, gray, thin] (y1);
    ...
    \path (x3) edge [bend right=320, arrows = <->, gray, thin] (y2);
    
  }
```

The other alternative to the traditional cross-lagged panel model that actually controls for unobserved stable heterogeneity is @hamaker2015critique's Random Intercept Cross-Lagged Panel Model [RI-CLPM; see also @mulder2021three]. To ensure that the model only takes into account change over time, the RI-CLPM explicitly models within-unit and between-unit variances as separate attributes. Similar to @allison2017maximum's DPM, the first step involves estimating a latent variable that captures all stable variation between units---again drawing on the latent growth curve model specification, this latent variable is defined by all observations $Y_t$ over time with factor loadings contrained to 1, essentially creating a random intercept for $Y$ that varies between units.

While @allison2017maximum's approach involves allowing this latent variable to covary with pretedermined regressors---which implies treating it as a fixed effect that captures all unobserved stable heterogeneity---the RI-CLPM instead repeats the process and creates another latent variable defined by all observations $X_t$ over time, thus creating a random intercept for $X$ that varies between units. The two latent variables reflecting random intercepts for $Y$ and $X$ (represented by $\alpha_Y$ and $\alpha_X$ in Figure @riclpm) are allowed to covary, which ensures that they capture all unobserved stable heterogeneity across both variables. Figure @riclpm depicts the the RI-CLPM.

```{tikz riclpm, fig.cap = "Random Intercept Cross-Lagged Panel Model (Hamaker et al., 2015)"}
\usetikzlibrary{positioning}
\definecolor{black}{HTML}{000000}
\tikzset{
    > = stealth,
...
\tikz{
    \node[manifest] (x1) at (0,5) {$X_1$};
    \node[manifest] (x2) at (2,5) {$X_2$};
    \node[manifest] (x3) at (4,5) {$X_3$};
    ...
    \node[manifest] (y2) at (2,1) {$Y_2$};
    \node[manifest] (y3) at (4,1) {$Y_3$};
    
    \node [hidden] (bx1) at (-3,4.5) {$\alpha_X$};
    \node [hidden] (by1) at (-3,1.5) {$\alpha_Y$};
